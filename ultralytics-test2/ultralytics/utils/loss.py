...
class BboxLoss(nn.Module):
    ...
    def forward(
        ...
        # 2. 创新点 3b：计算 NWD (Normalized Gaussian Wasserstein Distance)
        # 这里的 pred_bboxes[fg_mask] 和 target_bboxes[fg_mask] 形状均为 [N, 4]
        nwd = bbox_nwd(pred_bboxes[fg_mask], target_bboxes[fg_mask])

        # 3. 融合 IoU 与 NWD
        # 比例建议：0.7 IoU + 0.3 NWD (针对细胞这类极小目标，可尝试 0.5:0.5)
        iou_fused = 0.9 * iou + 0.1 * nwd.view_as(iou)

        # 4. 计算最终损失
        loss_iou = ((1.0 - iou_fused) * weight).sum() / target_scores_sum
