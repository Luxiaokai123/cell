...
def bbox_nwd(box1, box2, constant=12.1):
    """
    计算归一化高斯 Wasserstein 距离 (NWD)
    box1/box2: [N, 4] (x1, y1, x2, y2)
    constant: 归一化常数，通常取值范围在 10-20 之间，推荐 12.1
    """
    # 转换成中心点和宽高 (cx, cy, w, h)
    b1_x1, b1_y1, b1_x2, b1_y2 = box1.chunk(4, -1)
    b2_x1, b2_y1, b2_x2, b2_y2 = box2.chunk(4, -1)
    ...
    # 计算高斯分布中心点距离的平方
    t1 = (cx1 - cx2) ** 2 + (cy1 - cy2) ** 2
    # 计算宽高分布的 Wasserstein 距离项
    t2 = ((w1 - w2) / 2) ** 2 + ((h1 - h2) / 2) ** 2
    
    # 计算 Wasserstein 距离
    wasserstein = torch.sqrt(t1 + t2 + 1e-7)
