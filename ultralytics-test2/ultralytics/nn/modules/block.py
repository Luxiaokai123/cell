...
class DEConv(nn.Module):
    def __init__(self, dim):
        ...
        # 1. 定义邻域权重 (3x3 深度可分离卷积核)
        self.weight_diff = nn.Parameter(torch.empty(dim, 1, 3, 3))
        # 2. 定义中心权重 (每个通道一个独立中心)
        self.weight_center = nn.Parameter(torch.empty(dim, 1, 1, 1))
        
        # 初始化权重 (建议使用 Kaiming 初始化或正态分布)
        nn.init.kaiming_normal_(self.weight_diff, mode='fan_out', nonlinearity='relu')
        nn.init.normal_(self.weight_center, std=0.01)
        
        # 调节系数 theta: 0.0 表示纯标准卷积, 1.0 表示纯差分卷积
        self.theta = nn.Parameter(torch.tensor(0.1)) 

    def forward(self, x):
        # 获取当前权重的邻域总和 [dim, 1, 1, 1]
        # 注意：在差分卷积理论中，中心位置的值应该被剔除或抵消
        sum_diff = self.weight_diff.sum(dim=(2, 3), keepdim=True)
        
        # 构造最终卷积核
        # 核心公式：kernel = weight_diff (邻域) + (weight_center - theta * sum_diff) (中心补偿)
        kernel = self.weight_diff.clone()
        
        # 找到中心点索引 [1, 1] 并进行重参数化修改
        center_val = self.weight_center - self.theta * sum_diff
        kernel[:, :, 1, 1] = kernel[:, :, 1, 1] + center_val.squeeze(-1).squeeze(-1)
        
        # 执行深度卷积 (groups=dim)
        return F.conv2d(x, kernel, padding=1, groups=self.dim)

class DEBottleNeck(nn.Module):
    expansion = 1
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5, act=True, stride=1, **kwargs):
        super().__init__()
        c_ = int(c2 * e) 
        ...
        self.cv1 = nn.Sequential(
            ...
            nn.BatchNorm2d(c_),
            nn.ReLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        )
        
        # 2. 第二层：差分卷积 (DEConv) + 归一化 + 激活
        # 注意：DEConv 内部通常已经包含了卷积逻辑，这里加上 BN 稳住分布
...
class SPDConv(nn.Module):
    ...
    def forward(self, x):
        # Space-to-depth: [bs, c, h, w] -> [bs, 4c, h/2, w/2]
        return torch.cat([x[..., ::2, ::2], x[..., 1::2, ::2], x[..., ::2, 1::2], x[..., 1::2, 1::2]], self.d)

# 3. Agent_AIFI (创新点2)
class Agent_AIFI(nn.Module):
    def __init__(self, c1, c2, nc=8, **kwargs): 
        super().__init__()
        self.dim = c1 
        self.out_dim = c2 
        self.num_agents = nc
        ...
        # 2. 改进 Agent Tokens 初始化
        self.agent_tokens = nn.Parameter(torch.zeros(1, nc, c1))
        nn.init.trunc_normal_(self.agent_tokens, std=.02)
        
        # 3. 输出投影
        self.proj = nn.Linear(c1, c2)
        self.dropout = nn.Dropout(0.1)
        
        # 4. 残差投影：确保 c1 和 c2 通道对齐
        # 使用 Linear 因为 forward 中处理的是 flatten 后的序列
        if c1 != c2:
            self.shortcut_proj = nn.Linear(c1, c2)
        else:
    ...
    def forward(self, x):
        if len(x.shape) == 4:
            ...
            # --- 保存残差分支 (先对齐维度) ---
            shortcut = self.shortcut_proj(x_flatten)
            
            # --- 进入注意力流程 ---
            x_norm = self.norm(x_flatten)
            
            q = self.q(x_norm)
            k = self.k(x_norm)
            v = self.v(x_norm)
            
            # Agent Attention Core
            scale = self.dim ** -0.5
            
            # 1. 像素特征聚合到 Agent (Pooling)
            attn_agent = (self.agent_tokens @ k.transpose(-2, -1)) * scale
            attn_agent = attn_agent.softmax(dim=-1)
            agent_v = attn_agent @ v
            
            # 2. Agent 特征广播回像素 (Broadcasting)
            attn_q = (q @ self.agent_tokens.transpose(-2, -1)) * scale
            ...
            # 此时 out 和 shortcut 形状均为 [b, h*w, c2]
            out = out + shortcut
            
            # 回复原始形状 [b, c2, h, w]
            return out.transpose(1, 2).reshape(b, self.out_dim, h, w)
        return x